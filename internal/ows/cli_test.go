...
func TestSendUnsignedTxBuildsOwsCommand(t *testing.T) {
	...
	runCommandFunc = func(_ context.Context, bin string, args []string, env []string) ([]byte, []byte, error) {
		...
		return []byte(`{"txHash":"0x1111111111111111111111111111111111111111111111111111111111111111"}`), nil, nil
	}
	...
	if result.TxHash != "0x1111111111111111111111111111111111111111111111111111111111111111" {
		t.Fatalf("unexpected tx hash %q", result.TxHash)
	}
...
func TestParseSendTxResultRejectsMalformedTxHash(t *testing.T) {
	_, err := parseSendTxResult([]byte(`{"txHash":"0xabc123"}`), "eip155:1")
	if err == nil {
		t.Fatal("expected malformed tx hash to fail")
	}
}

func TestSendUnsignedTxRejectsMalformedTxHash(t *testing.T) {
	t.Setenv(EnvOWSToken, "test-passphrase")
