...
func (c *Client) YieldOpportunities(ctx context.Context, req providers.YieldRequest) ([]model.YieldOpportunity, error) {
	...
	maxRiskScore := yieldutil.RiskOrder(req.MaxRisk)
	if maxRiskScore == 0 {
		maxRiskScore = yieldutil.RiskOrder("high")
	}
	...
	for _, p := range pools {
		...
		if yieldutil.RiskOrder(riskLevel) > maxRiskScore {
			continue
		}
		if apyTotal == 0 || tvl == 0 {
			warnings++
		...
		liq := tvl
		score := yieldutil.ScoreOpportunity(apyTotal, tvl, liq, riskLevel)
