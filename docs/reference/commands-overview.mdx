...
- Standard EVM `plan` commands are wallet-first and use `--wallet`.
- `--from-address` remains available as deprecated compatibility for planning.
- Tempo swap planning is the explicit exception and still uses `--from-address`.
