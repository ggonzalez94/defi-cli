...
- legacy signer flags are deprecated compatibility for `legacy_local` actions only
...
- `--from-address` remains documented only as deprecated compatibility for planning.
...
Rewards planning follows the same OWS-first rules as lend and yield: prefer `--wallet`, keep `--from-address` only for deprecated compatibility, and use `DEFI_OWS_TOKEN` for wallet-backed submit.
