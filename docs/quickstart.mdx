...
For new standard EVM actions, `--wallet` is the primary planning identity. `--from-address` remains as deprecated compatibility only. Tempo is the explicit exception and still uses `--from-address`.
...
Wallet-backed submit uses the action's persisted `wallet_id`. It does not take owner-mode private keys or legacy signer flags.
