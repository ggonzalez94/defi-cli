...
Standard EVM yield planning is wallet-first. `--from-address` remains as deprecated compatibility only, and wallet-backed submit uses persisted wallet metadata plus `DEFI_OWS_TOKEN`.
