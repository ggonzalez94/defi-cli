...
Bridge execution is available through `bridge plan|run|submit|status` for `--provider across|lifi`.
...
- Across `/swap/approval` responses can include max-allowance `approve` calldata. `defi-cli` blocks approval amounts above planned input by default; use `--allow-max-approval` on `bridge run|submit` only when you explicitly accept that larger allowance.
- Bridge `run|submit` marks bridge steps complete only after destination settlement is confirmed by provider status APIs (Across `/deposit/status`, LiFi `/status`).
...
- Global `--timeout` also bounds the command. Set `--timeout` high enough for end-to-end execution (for example `--timeout 20m` when `--step-timeout 15m`).
