...
Standard EVM bridge planning is wallet-first. `--from-address` remains available as deprecated compatibility only, and wallet-backed submit uses the persisted `wallet_id` plus `DEFI_OWS_TOKEN`.
